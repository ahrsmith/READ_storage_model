...
#test case 
methane_per_day = 421098.9129 #L METHANE PRODUCED

#technical assumptions
energy_density = 36 #MJ per m^3; this is for METHANE
conv_MJ_to_kWh = 1 / 3.6 #MJ per m^3 to kWh per m^3
conv_m3_to_L = 1/1000 #kWh per m^3 to kWh per L
energy_density = energy_density*conv_MJ_to_kWh*conv_m3_to_L #final kWh per L
turbine_eff = .4
...
for (gen_hours in 1:24) {
  ...
  max_values$max_storage_capacity = max_values$max_storage_capacity*methane_per_day/24
  ...
  min_size_generator = methane_per_day * energy_density * turbine_eff / gen_hours ##biogas in L; energy density in kWh per L; gen_hours in hr; final in kW
